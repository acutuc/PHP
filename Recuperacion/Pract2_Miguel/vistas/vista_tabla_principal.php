...
<form id="form_regs_buscar" action="index.php" method="post">
    <div>
        ...
        <select id="registros" name="registros" onchange="document.getElementById('form_regs_buscar').submit();">
            <option <?php if ($_SESSION["registros"] == 3)
                echo "selected"; ?> value="3">3</option>
            <option <?php if ($_SESSION["registros"] == 6)
                echo "selected"; ?> value="6">6</option>
            <option <?php if ($_SESSION["registros"] == -1)
                echo "selected"; ?> value="-1">TODOS</option>
        </select>
    </div>
    <div>
        <input type="text" name="buscar" value="<?php echo $_SESSION["buscar"]; ?>" /> <button
...
foreach ($usuarios as $tupla) {
    ...
    echo "<td><form action='index.php' method='post'><button class='enlace' name='btnListar' value='" . $tupla["id_usuario"] . "'>" . $tupla["nombre"] . "</button></form></td>";
    echo "<td><form action='index.php' method='post'><input type='hidden' name='foto' value='".$tupla["foto"]."'/><button class='enlace' name='btnBorrar' value='" . $tupla["id_usuario"] . "'>Borrar</button> - <button class='enlace' name='btnEditar' value='" . $tupla["id_usuario"] . "'>Editar</button></form></td>";
    echo "</tr>";
}
